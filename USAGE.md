...
`GlideGrail.md` is just a markdown file — wiring it into an assistant always comes down to
...
| **Skill (`SKILL.md`)** | ~100 tokens until the model decides it's relevant, then loads on demand | Low and elastic | On-demand | **Large references like this one**; multi-tool setups |
...
The skill is two files — [`SKILL.md`](./SKILL.md) and `GlideGrail.md` — kept together in a
folder:
...
### ChatGPT (app)
- **Project:** upload `GlideGrail.md` and add *"Follow GlideGrail.md for all ServiceNow code"*
  to the Project instructions. Files and instructions carry across every chat in the Project.
- **Custom GPT:** add `GlideGrail.md` as a knowledge file and reference it in the GPT's
...
- **Gemini app — Gem:** create a Gem, add *"Follow GlideGrail.md for all ServiceNow code"* to
  its instructions, and upload `GlideGrail.md` as a context file.
...
### Any other assistant
- Drop an **`AGENTS.md`** at the repo root pointing to `GlideGrail.md` — Cursor, Copilot, and
